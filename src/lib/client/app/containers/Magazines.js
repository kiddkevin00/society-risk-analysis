...
class UnconnectedMagazines extends Component {
    ...
    return (
      <div key={magazine.key} className={className}>
        <article className="article-entry">
          ...
          <div className="desc">
            <p>
              {`${magazine['期刊年份']} 第${magazine['期刊編號']}期`}
            </p>
            <h2>
  ...
  render() {
    ...
    const magazinesColumns = this.props.magazines
      .map((magazine) => this.renderMagazineColumns(magazine));
...
const mapStateToProps = state => ({
  ...
  magazines: (state.firebase.ordered && Array.isArray(state.firebase.ordered['學會會刊'])) ?
    state.firebase.ordered['學會會刊'].map((magazine) => ({ key: magazine.key, ...magazine.value })) : [],
});
